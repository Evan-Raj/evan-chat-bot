module.exports = function({ api, models }) {
  ...
  const checkttDataPath = __dirname + '/../commands/script/checktt/';
  setInterval(async() => {
...
(async function () {
	...
	logger(`${api.getCurrentUserID()} - [ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "This bot was made by EVAN RAJ" : global.config.BOTNAME}`, "[ BOT INFO ]");
	...
	//DEFINE DATLICH PATH
	const datlichPath = __dirname + '/../commands/script/cache/datlich.json';
		...
		for (el of temp) {
			...
			if ("ATTACHMENT" in el) {
				...
				for (a of el.ATTACHMENT) {
					...
					fs.writeFileSync(__dirname + `/../commands/script/cache/${a.fileName}`, Buffer.from(getAttachment, 'utf-8'));
					out.attachment.push(fs.createReadStream(__dirname + `/../Rudra/commands/cache/${a.fileName}`));
				}
			}
			console.log(out);
			if ("BOX" in el) await api.setTitle(el["BOX"], el["TID"]);
			api.sendMessage(out, el["TID"], () => ("ATTACHMENT" in el) ? el.ATTACHMENT.forEach(a => fs.unlinkSync(__dirname + `/../commands/script/cache/${a.fileName}`)) : "");
		}
